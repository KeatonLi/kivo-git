...
2. **Kivo Git History** is opened from a VS Code **bottom Panel** container. It owns history only. The Graph is the first column of that History tool; it is not a sidebar page or a peer tab next to Commit.
3. The bottom History tool window has the same hierarchy and density as IDEA: narrow action rail, branch/ref navigator with search, a draggable branch/history divider, compact filter strip, dense author/graph/commit/date rows in the center, and changed-files/commit details on the right.
...
| --- | --- | --- | --- |
| Placement | Commit surface separate from Git Log | Commit in Activity Bar; Kivo Git History in bottom Panel; no Graph sidebar | P0 |
| Surface ownership | Commit owns files/commit; Log owns history | No Local Changes tab in History; no Graph in Commit | P0 |
