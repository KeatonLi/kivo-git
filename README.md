...
Kivo Git is for developers who want a focused Git tool window in VS Code: persistent changelists, a useful branch popup, and a real branch-aware history graph.
...
Kivo Git borrows the best IDE workflow ideas without copying another product's pixels. Git operations use the local `git` executable, so GitHub, GitLab, Gitee, Bitbucket, and internal remotes work without Kivo Git storing credentials.
